...
typename list<T>::iterator list<T>::begin() {
  if (endMarker == nullptr) CreateEndNode();
...
void list<T>::unique() {
  if (head != nullptr) {
    ...
    for (size_type i = 0; i < size_ - 1; ++i) {
      if (current->value != next->value) uniqueList.push_back(current->value);
...
list<T>::const_iterator::operator++() noexcept {
  return itr_->next ? itr_ = itr_->next : itr_ = itr_end_;
}

template <typename T>
typename list<T>::const_iterator
list<T>::const_iterator::operator--() noexcept {
  return itr_->prev ? itr_ = itr_->prev : itr_ = itr_end_;
}

template <typename T>
typename list<T>::const_iterator list<T>::const_iterator::operator+(
    const size_type step) noexcept {
  for (size_type i = 0; i < step; ++i) ++*this;
  return *this;
}
...
typename list<T>::iterator list<T>::iterator::operator+(
    const size_type step) noexcept {
  iterator ret = this->itr_;
  for (size_type i = 0; i < step; ++i) ++ret;
  return ret;
}
...
#endif  // S21_CONTAINERS_S21_LIST_TPP_
